...
export default function POS() {
  ...
  const [receipt,    setReceipt]    = useState(null); // venta confirmada
  ...
  const filtered = products.filter(p => {
    ...
    const matchName = p.name_products?.toLowerCase().includes(search.toLowerCase());
  ...
  const addToCart = product => {
    setCart(prev => {
      ...
      if (existing) return prev.map(i => i.id_products === product.id_products ? { ...i, qty: i.qty + 1 } : i);
  ...
  const updateQty = (id, delta) => {
    setCart(prev => {
      const next = prev.map(i => i.id_products === id ? { ...i, qty: Math.max(0, i.qty + delta) } : i);
      return next.filter(i => i.qty > 0);
    });
  };

  const removeFromCart = id => setCart(prev => prev.filter(i => i.id_products !== id));
  ...
  const handleCheckout = async () => {
    ...
    try {
      const saleData = {
        ...
        items: cart.map(i => ({ productId: i.id_products, quantity: i.qty })),
      };
    ...
    <div className="pos-shell">
      ...
      <div className="pos-left">
        ...
        <div className="pos-search-bar">
          ...
          />
          {search && <button className="pos-search-clear" onClick={() => setSearch("")}><span className="material-symbols-outlined">close</span></button>}
        </div>
        ...
        <div className="pos-cats">
          ...
          {categories.map(c => (
            <button key={c.id} className={`pos-cat-btn ${String(activeCat) === String(c.id) ? "active" : ""}`} onClick={() => setActiveCat(c.id)}>
        ...
        {loading ? (
          ...
          <div className="pos-grid">
            ...
            {filtered.map(p => {
              const inCart = cart.find(i => i.id_products === p.id_products);
              return (
                <button key={p.id_products} className={`pos-product-card ${inCart ? "in-cart" : ""}`} onClick={() => addToCart(p)}>
                  {p.imge_url
                    ? <img src={p.imge_url} alt={p.name_products} className="pos-product-img" />
                    : <div className="pos-product-img-placeholder"><span className="material-symbols-outlined">restaurant</span></div>
                  }
                  {inCart && <span className="pos-cart-badge">{inCart.qty}</span>}
                  <div className="pos-product-info">
                    <span className="pos-product-name">{p.name_products}</span>
      ...
      <div className="pos-right">
        ...
        <div className="pos-cart-items">
          ...
          {cart.map(item => (
            <div key={item.id_products} className="pos-cart-item">
              <div className="pos-cart-item-info">
                <span className="pos-cart-item-name">{item.name_products}</span>
                <span className="pos-cart-item-price">${(item.price * item.qty).toLocaleString()}</span>
              </div>
              <div className="pos-cart-item-controls">
                <button className="qty-btn" onClick={() => updateQty(item.id_products, -1)}>−</button>
                <span className="qty-value">{item.qty}</span>
                <button className="qty-btn" onClick={() => updateQty(item.id_products, +1)}>+</button>
                <button className="qty-btn remove" onClick={() => removeFromCart(item.id_products)}>
        ...
        <div className="receipt-items">
          {receipt.cartSnapshot.map((item, i) => (
            <div key={i} className="receipt-item">
              <span>{item.name_products} × {item.qty}</span>
