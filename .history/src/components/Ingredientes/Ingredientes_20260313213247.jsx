...
const EMPTY = { name_ingredients: "", description: "", active: true };
...
export default function Ingredientes() {
  ...
  const filtered = rows.filter(r =>
    r.name_ingredients?.toLowerCase().includes(search.toLowerCase())
  );

  const openAdd  = () => { setForm(EMPTY); setEditId(null); setModal("form"); };
  const openEdit = row => {
    setForm({ name_ingredients: row.name_ingredients ?? "", description: row.description ?? "", active: row.active ?? true });
    setEditId(row.id_ingredients);
    setModal("form");
  };
  ...
  const handleSave = async e => {
    ...
    setSaving(true);
    try {
      editId ? await updateIngredient(editId, form) : await createIngredient(form);
      setModal(null);
      load();
        ...
        <table className="crud-table">
          ...
          <tbody>
            ...
            {!loading && filtered.map(row => (
              <tr key={row.id_ingredients}>
                <td style={{ fontWeight: 500 }}>{row.name_ingredients}</td>
                <td style={{ color: "var(--text-2)", maxWidth: 300, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.description ?? "—"}</td>
                <td>
                  {row.active
                ...
                <td>
                  <div className="row-actions">
                    ...
                    <button className="action-btn delete" onClick={() => setConfirmDel(row.id_ingredients)} title="Eliminar">
        ...
        <Modal title={editId ? "Editar ingrediente" : "Nuevo ingrediente"} onClose={() => setModal(null)}>
          <form onSubmit={handleSave}>
            <div className="form-group">
              ...
              <input className="form-input" required value={form.name_ingredients} onChange={e => setForm(f => ({ ...f, name_ingredients: e.target.value }))} placeholder="Pan brioche" />
            </div>
            <div className="form-group">
              <label className="form-label">Descripción</label>
              <textarea className="form-textarea" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Descripción del ingrediente..." />
            </div>
